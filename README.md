...
- 🎯 Entry points to execute your program using `python -m <test_fastapi>` or `$ test_fastapi` with basic CLI argument parsing.
...
[![codecov](https://codecov.io/gh/tuhoang1412/test-fastapi/branch/main/graph/badge.svg?token=test-fastapi_token_here)](https://codecov.io/gh/tuhoang1412/test-fastapi)
[![CI](https://github.com/tuhoang1412/test-fastapi/actions/workflows/main.yml/badge.svg)](https://github.com/tuhoang1412/test-fastapi/actions/workflows/main.yml)

Awesome test_fastapi created by tuhoang1412

## Install

from source
```bash
git clone https://github.com/tuhoang1412/test-fastapi test_fastapi
cd test_fastapi
make install
```
...
```bash
pip install test_fastapi
```

## Executing

```bash
$ test_fastapi run --port 8080
```

or

```bash
python -m test_fastapi run --port 8080
```
...
```bash
❯ test_fastapi --help
Usage: test_fastapi [OPTIONS] COMMAND [ARGS]...
...
```bash
❯ test_fastapi create-user --help
Usage: test_fastapi create-user [OPTIONS] USERNAME PASSWORD
...
```bash
test_fastapi create-user admin admin --superuser
```
...
Run with `test_fastapi run` and access http://127.0.0.1:8000/docs

![](https://raw.githubusercontent.com/rochacbruno/fastapi-project-template/master/docs/api.png)


**For some api calls you must authenticate** using the user created with `test_fastapi create-user`.
...
-----------------------------------------------------
test_fastapi/__init__.py              4      0   100%
test_fastapi/app.py                  16      1    94%
test_fastapi/cli.py                  21      0   100%
test_fastapi/config.py                5      0   100%
test_fastapi/db.py                   10      0   100%
test_fastapi/models/__init__.py       0      0   100%
test_fastapi/models/content.py       47      1    98%
test_fastapi/routes/__init__.py      11      0   100%
test_fastapi/routes/content.py       52     25    52%
test_fastapi/routes/security.py      15      1    93%
test_fastapi/routes/user.py          52     26    50%
test_fastapi/security.py            103     12    88%
-----------------------------------------------------
TOTAL                               336     66    80%
...
```py
from test_fastapi.config import settings
```
...
> `dynaconf_merge` is a boolean that tells if the settings should be merged with the default settings defined in test_fastapi/default.toml.

### As environment variables
```bash
export test_fastapi_KEY=value
export test_fastapi_KEY="@int 42"
export test_fastapi_KEY="@jinja {{ this.db.uri }}"
export test_fastapi_DB__uri="@jinja {{ this.db.uri | replace('db', 'data') }}"
```
...
```bash
test_fastapi_ENV=production test_fastapi run
```
